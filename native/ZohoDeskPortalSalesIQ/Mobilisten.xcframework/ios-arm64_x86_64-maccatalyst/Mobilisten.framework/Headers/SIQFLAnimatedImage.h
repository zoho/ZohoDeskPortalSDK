...
#import "SIQFLAnimatedImageView.h"
...
//
//  An `FLAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `SIQFLAnimatedImageView`.
...
//
@interface SIQFLAnimatedImage : NSObject
...
typedef NS_ENUM(NSUInteger, SIQFLLogLevel) {
    FLLogLevelNone = 0,
    FLLogLevelError,
    FLLogLevelWarn,
    FLLogLevelInfo,
...
@interface SIQFLAnimatedImage (Logging)

+ (void)setLogBlock:(void (^)(NSString *logString, SIQFLLogLevel logLevel))logBlock logLevel:(SIQFLLogLevel)logLevel;
+ (void)logStringFromBlock:(NSString *(^)(void))stringBlock withLevel:(SIQFLLogLevel)level;

@end

#define FLLog(logLevel, format, ...) [SIQFLAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]
