...
RE.setupMaxCount = function(){
    var max = RE.editor.attributes.maxLength.value;

    if (typeof max == "undefined") {// NO I18N
        return;
    }
    
    document.getElementById("editor").addEventListener("keydown", function(event){// NO I18N
        if (document.getElementById("editor").innerText.length >= max && event.keyCode != 8){// NO I18N
            event.preventDefault();
        }
    });
    
    RE.editor.onpaste = function(e){
        var warningLbl = document.getElementById("warningLbl");// NO I18N
        var max = RE.editor.getAttribute("maxLength");// NO I18N
        var contentLength = e.clipboardData.getData('text/plain').length + RE.editor.innerText.length;// NO I18N
        
        if (contentLength > max) {
            RE.editor.classList.add("hide");// NO I18N
            warningLbl.classList.remove("hide");// NO I18N
            
            setTimeout(function() {
                RE.editor.classList.remove("hide");// NO I18N
                warningLbl.classList.add("hide");// NO I18N
            }, 2000);
...
RE.initialHeight = function(){
    ...
    RE.callback("ready:"+Math.max(clientHeight, 50));// NO I18N
}

RE.postClientHeight = function(){
//    var clientHeight = Math.max(document.body.scrollHeight, document.body.offsetHeight,document.documentElement.clientHeight, document.documentElement.scrollHeight, document.documentElement.offsetHeight);
    
    RE.callback("clientHeight:"+Math.max(Math.round(RE.editor.getBoundingClientRect().height), 50));// NO I18N
}
...
RE.editor.addEventListener("input", function(event) {// NO I18N
    var text = RE.getText();
...
RE.callback = function(method) {
    window.webkit.messageHandlers.zdplatformWebEditornotifier.postMessage(method);
};
...
RE.setHtml = function(contents) {
    ...
    RE.postClientHeight();
    RE.disableChildNodes();
};

RE.disableChildNodes = function() {
    var childNodes = RE.editor.getElementsByTagName('*');
...
RE.getHtml = function() {
    return [RE.editor.innerHTML, RE.editor.innerText]
};
...
RE.setPlaceholderText = function(text) {
    RE.editor.setAttribute("data-text", text);// NO I18N
};
    ...
    if (sel.rangeCount) {
        ...
        if (range.getClientRects) {
            ...
            if (rects.length > 0) {
                ...
                let childRect = range.startContainer.getClientRects()
...
RE.enableDarkMode = function(){
    ...
    stylesheet.setAttribute('href', "ZDPDark.css");// NO I18N
}

RE.enableLightMode = function(){
    var stylesheet = document.getElementById("system");// NO I18N
    stylesheet.setAttribute('href', "ZDPLight.css");// NO I18N
}
...
RE.commendState = function(style) {
    return document.queryCommandState(style)
}

RE.enable = function(style) {
    if (!RE.commendState(style)) {
